...
async def get_fps(current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    return {"current": rtsp.current_fps, "max": rtsp.fps_max, "target": rtsp.target_fps}


@router.put("/video/fps")
async def set_fps(data: dict, current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
...
@router.get("/nn")
async def get_nn_config(current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    return {"show_osd": rtsp.show_osd, "classes": rtsp.supported_class}


@router.put("/nn")
async def update_nn(data: dict, current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    if "enable" in data:
...
@router.get("/nn/skip")
async def get_nn_skip(current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    if not rtsp:
...
@router.put("/nn/skip")
async def update_nn_skip(data: SkipUpdateSchema, current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    if not rtsp:
        raise HTTPException(status_code=503, detail="RTSP server not ready")
...
@router.get("/nn/log")
async def get_nn_log(current_user=Depends(get_current_user), rtsp=Depends(get_rtsp)):
    if not rtsp:
        return []
