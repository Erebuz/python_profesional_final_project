...
class AuthMixin:
    ...
    @staticmethod
    def create_test_user(username: str = "testuser", password: str = "testpass", user_id: int = 1) -> dict:
    ...
    def create_test_users(count: int = 3) -> list[dict]:
        ...
        return [AuthMixin.create_test_user(f"user{i}", f"pass{i}", i) for i in range(1, count + 1)]
...
class TestAuth(AuthMixin):
    ...
    @pytest.mark.parametrize("plain,hashed", [
        ("testpass", bcrypt.hashpw("testpass".encode(), bcrypt.gensalt()).decode()),
        ("password123", bcrypt.hashpw("password123".encode(), bcrypt.gensalt()).decode()),
    ])
    ...
    @pytest.mark.parametrize("plain,hashed", [
        ("wrongpass", bcrypt.hashpw("rightpass".encode(), bcrypt.gensalt()).decode()),
        ("test", bcrypt.hashpw("different".encode(), bcrypt.gensalt()).decode()),
    ])
    ...
    def test_get_user_by_username(self, username, user_id, temp_auth_file, test_users):
    ...
    def test_get_user_by_id(self, user_id, username, temp_auth_file, test_users):
    ...
    @pytest.mark.parametrize("user_id,update_data,should_succeed", [
        (1, {"username": "newname"}, True),
        (1, {"password": "newpass"}, True),
        (999, {"username": "nonexistent"}, False),  # Non-existent user
    ])
    def test_update_user(self, user_id, update_data, should_succeed, temp_auth_file, test_users):
...
class TestAuthRoutes(AuthMixin):
    ...
    def client(self, temp_auth_file):
        ...
        with patch('web.auth_routes.auth_service') as mock_auth:
            mock_auth.get_user_by_username = Mock(return_value=self.create_test_user())
    ...
    def test_me_success(self, client):
        ...
        with patch('web.auth_routes.auth_service') as mock_auth:
            mock_auth.get_user_by_id = Mock(return_value=self.create_test_user())
    ...
    def test_catch_all_file_exists(self, client):
        ...
        with patch('web.sys_routes.os.path.exists', return_value=True):
            with patch('web.sys_routes.os.path.isfile', return_value=True):
                with patch('web.sys_routes.FileResponse') as mock_file_response:
