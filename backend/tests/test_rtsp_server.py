...
class FrameMixin:
    ...
    @staticmethod
    def create_colored_frame(width: int = 640, height: int = 480, color: tuple = (255, 0, 0)) -> np.ndarray:
...
class NeuralMixin:
    ...
    def create_mock_neural_network():
        ...
        mock_net = Mock()
        mock_net.class_names = ["person", "car", "bicycle", "motorbike", "bus", "truck"]
        mock_net.detect_objects = Mock(return_value=NeuralMixin.create_mock_neural_output())
        return mock_net
...
class CaptureMixin:
    ...
    def create_mock_capture(width: int = 640, height: int = 480):
        ...
        mock_capture.read.return_value = (True, FrameMixin.create_test_frame(width, height))
        return mock_capture
...
class TestRtspStreamer(FrameMixin, ConfigMixin):
    ...
    @pytest.mark.parametrize("port,host,uri,expected_url", [
        (8554, "localhost", "video", "rtsp://localhost:8554/video"),
        (8555, "192.168.1.1", "stream1", "rtsp://192.168.1.1:8555/stream1"),
        (9000, "example.com", "cam", "rtsp://example.com:9000/cam"),
    ])
    def test_rtsp_url_format(self, port, host, uri, expected_url, mock_capture):
    ...
    @pytest.mark.parametrize("env_width,env_height,expected_width,expected_height", [
        ("1280", "720", 1280, 720),
        ("1920", "1080", 1920, 1080),
        (None, None, 640, 480),  # значения по умолчанию из мока
    ])
    def test_frame_dimensions_from_env(self, env_width, env_height, expected_width, expected_height, mock_capture):
    ...
    def test_get_ffmpeg_command(self, mock_capture):
        ...
        with patch('cv2.VideoCapture', return_value=mock_capture):
            streamer = RtspStreamer(source=0, fps=30, port=8554, host="localhost", uri="video")
...
class TestRtspMergeServer(FrameMixin, ConfigMixin, NeuralMixin):
    ...
    @pytest.mark.parametrize("enabled_classes,expected_whitelist_count", [
        ({"person": True, "car": True, "bicycle": False, "motorbike": False, "bus": False, "truck": False, "boat": False, "backpack": False, "handbag": False, "cell phone": False}, 2),
        ({"person": True, "car": False, "bicycle": False, "motorbike": False, "bus": False, "truck": False, "boat": False, "backpack": False, "handbag": False, "cell phone": False}, 1),
        ({"person": False, "car": False, "bicycle": False, "motorbike": False, "bus": False, "truck": False, "boat": False, "backpack": False, "handbag": False, "cell phone": False}, 0),
    ])
    def test_whitelist_property(self, enabled_classes, expected_whitelist_count, mock_capture):
    ...
    def test_frame_update_with_osd(self, test_frame, mock_capture, mock_neural_network):
    ...
    @pytest.mark.parametrize("frame_skip,initial_counter,expected_calls", [
        (0, 0, 1),  # без пропуска - вызов на каждом кадре
        (1, 0, 1),  # пропуск 1 кадра
        (2, 0, 1),  # пропуск 2 кадра
    ])
    def test_frame_skip_logic(self, frame_skip, initial_counter, expected_calls, test_frame, mock_capture, mock_neural_network):
        """Тест логики пропуска кадров."""
        with patch('cv2.VideoCapture', return_value=mock_capture):
            server = RtspMergeServer(source=0, show_osd=True, frame_skip=frame_skip)
            server.model = mock_neural_network
            server.frame_counter = initial_counter
            
            server.frame_update(test_frame)
            
            assert mock_neural_network.detect_objects.call_count == expected_calls
    ...
    def test_draw_outputs(self, test_frame, mock_capture):
        ...
        with patch('cv2.VideoCapture', return_value=mock_capture):
            ...
            class_names = ["person", "car", "bicycle", "motorbike", "bus", "truck"]
    ...
    @pytest.mark.parametrize("white_list,expected_classes", [
        (["person"], ["person"]),
        (["person", "car"], ["person", "car"]),
        (None, ["person", "car"]),  # все классы из мока
    ])
    def test_draw_outputs_with_whitelist(self, white_list, expected_classes, test_frame, mock_capture):
        ...
        with patch('cv2.VideoCapture', return_value=mock_capture):
            ...
            result = server.draw_outputs(test_frame, outputs, class_names, white_list)
            
            if white_list is not None:
                for cl in server.active_class:
                    assert cl in expected_classes
    
    @pytest.mark.parametrize("active_class,expected_total", [
        ({"person": 1, "car": 2}, 3),
        ({"person": 5}, 5),
        ({}, 0),
    ])
    def test_update_activity_log(self, active_class, expected_total, mock_capture):
    ...
    @pytest.mark.parametrize("log_length,updates_count,expected_length", [
        (5, 3, 3),
        (5, 10, 5),
        (10, 15, 10),
    ])
    def test_activity_log_length_limit(self, log_length, updates_count, expected_length, mock_capture):
...
class TestRtspServerIntegration(FrameMixin, ConfigMixin, NeuralMixin):
    ...
    @pytest.mark.parametrize("config_class,extra_params", [
        (RtspStreamer, {}),
