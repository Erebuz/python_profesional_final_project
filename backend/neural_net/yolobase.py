...
class YoloBase:
    ...
    def __init__(self, model_path: str = "yolo11n.pt") -> None:
        self.device: Final[str] = "cuda" if torch.cuda.is_available() else "cpu"
    ...
    def predict(self, frame: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        """
        Выполняет детекцию и возвращает данные в формате,
        совместимом с вашим RtspMergeServer.
        """
        results = self.model.predict(frame, conf=0.45, iou=0.5, verbose=False, device=self.device)  # Порог уверенности  # Порог NMS
        ...
        for box in result.boxes:
            ...
            # Возвращаем массивы
        return (np.array([boxes], dtype=np.float32), np.array([scores], dtype=np.float32), np.array([classes], dtype=np.float32), np.array([len(classes)], dtype=np.int32))
